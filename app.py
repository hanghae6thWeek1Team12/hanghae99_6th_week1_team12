...
def home():
    ...
    menus = [
        {'menu': '순대국밥', 'img': '/static/image/sundae_soup.jpg'},
        {'menu': '소머리국밥', 'img': '/static/image/cow_soup.jpg'},
        {'menu': '굴국밥', 'img': '/static/image/oyster_soup.jpg'},
        {'menu': '돼지국밥', 'img': '/static/image/pork_soup.jpg'},
        {'menu': '따로국밥', 'img': '/static/image/separate_soup.jpg'},
        {'menu': '콩나물국밥', 'img': '/static/image/bean_soup.jpg'},
        {'menu': '평양온반', 'img': '/static/image/onban_soup.jpg'},
        {'menu': '수구레국밥', 'img': '/static/image/sugure_soup.jpg'},
    ]
...
def api_register():
    ...
    if id_receive == '':
        ...
        return jsonify({'result': 'empty', 'msg': '비밀번호를 확인해주세요.'})
    elif nickname_receive == '':
        return jsonify({'result': 'empty', 'msg': '닉네임을 입력해주세요.'})
    elif address_receive == '':
        return jsonify({'result': 'empty', 'msg': '주소를 입력해주세요.'})
    elif gender_receive == '성별을 선택해주세요.':
        ...
        if pw_receive != pw2_receive:
            ...
            nick_check = db.user.find_one({'nick': nickname_receive})
            if nick_check is not None:
                return jsonify({'result': 'nickerror', 'msg': '중복된 닉네임이 있습니다.'})
            ...
            # 찾지 못하면
                ...
                db.user.insert_one(
                    {'id': id_receive, 'pw': pw_hash, 'nick': nickname_receive, 'address': address_receive,
